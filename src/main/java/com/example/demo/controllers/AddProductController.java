        ...
        if (product.getId() != 0) {
            ...
            if (inventoryDifference > 0) {
                ...
                for (Part p : product2.getParts()) {
                    int newInv = p.getInv() - inventoryDifference;
                    if (newInv < p.getMinInv()) {
                        bindingResult.rejectValue("inv", "error.product", "Updating this product will lower part inventory below minimum limit");
                    }
                }
            }
        } else {
